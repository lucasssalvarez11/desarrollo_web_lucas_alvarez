...
const poblarRegiones = () => {
    ...
    // Limpiamos el select por si ya tiene opciones previas
    regionSelect.innerHTML = '<option value="">Seleccione una región</option>';

    data.regiones.forEach(region => {
        let option = document.createElement("option");
        option.value = region.numero; // Guardamos el número de la región como valor
...
const updateComunas = () => {
    ...
    let selectedRegionNumber = parseInt(regionSelect.value); // Convertimos a número

    // Limpiar el select de comunas
    comunaSelect.innerHTML = '<option value="">Seleccione una comuna</option>';

    // Buscar la región seleccionada en el array de regiones
    let regionEncontrada = data.regiones.find(region => region.numero === selectedRegionNumber);

    if (regionEncontrada) {
        regionEncontrada.comunas.forEach(comuna => {
            let option = document.createElement("option");
            option.value = comuna.id; // Guardamos el ID de la comuna como valor
...
function changeArguments() {
    const courseSelect = document.getElementById("contactarx");
    const reasonLabel = document.querySelector("label[for='url']");
    const reasonTextarea = document.getElementById("url");
  ...
  function changeArguments1() {
    const courseSelect = document.getElementById("tema");
    const reasonLabel = document.querySelector("label[for='otro']");
    const reasonTextarea = document.getElementById("otro");
...
document.getElementById("contactarx").addEventListener("change", changeArguments);
document.getElementById("tema").addEventListener("change", changeArguments1);

// Llenar las regiones al cargar la página
window.onload = () => {
    poblarRegiones();
};
